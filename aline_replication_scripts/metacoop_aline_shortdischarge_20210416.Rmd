...
metacoop_fit = weighted_fit_glmnet
...
for (k in 1:numstudies){
  ...
  list_of_crossfit_augmentations_rf_scaled[[k]] = crossfit_augmentation_rf(x = scaled_data_list[[k]],
                                                                 ...
                                                                 trt = trt_list[[k]],
                                                                 prop_score_x = list_of_crossfit_propensities[[k]],data_list
                                                                 use.crossfitting = TRUE,
                                                                 K=5, interactions = TRUE,
                                                                 trtcoding = "minusplus") 
  }
...
unweighted_weights_list <- vector(mode="list", length=numstudies)
for (k in 1:numstudies){
...
# e.g. try 1 through 10
scalingfactor = 6
plot(scalingfactor * metacoop_w_CIC_glmnet$concordance_kendall_tau %*% as.vector(metacoop_w_CIC_glmnet$numobsperstudy) - metacoop_w_CIC_glmnet$betasize_per_lambda_study %*% as.vector(log(metacoop_w_CIC_glmnet$numobsperstudy)))
