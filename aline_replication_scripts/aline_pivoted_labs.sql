DROP TABLE IF EXISTS aline_pivoted_lab CASCADE;
CREATE TABLE aline_pivoted_lab as
...
(
  ...
  WHERE
	...
	-- gases
	OR (lab.labname = 'paO2' and lab.labresult > 0)
	OR (lab.labname = 'paCO2' and lab.labresult > 0)
	OR (lab.labname = 'O2 Content' and lab.labresult > 0)
	OR (lab.labname = 'HCO3' and lab.labresult > 0)
	OR (lab.labname = 'O2 Sat (%)' and lab.labresult > 0)
)
select
    patientunitstayid
  ...
  , MAX(case when labname = 'O2 Content' then labresult else null end) as o2content
  , MAX(case when labname = 'HCO3' then labresult else null end) as hco3
  , MAX(case when labname = 'O2 Sat (%)' then labresult else null end) as o2sat
