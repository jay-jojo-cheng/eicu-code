...
DROP TABLE IF EXISTS aline_pivoted_vital CASCADE;
CREATE TABLE aline_pivoted_vital as
...
select
  ...
  , case
      when nursingchartcelltypevallabel = 'Invasive BP'
       ...
       and nursingchartvalue not in ('-','.')
          then cast(nursingchartvalue as numeric)
      -- other map fields (ORIGINAL PIVOT TABLE HAS THESE COMBINED, BUT WE SEPARATE THEM)
      --when nursingchartcelltypevallabel = 'MAP (mmHg)'
      -- and nursingchartcelltypevalname = 'Value'
      -- and nursingchartvalue ~ '^[-]?[0-9]+[.]?[0-9]*$'
      -- and nursingchartvalue not in ('-','.')
...
, avg(case when meanarterialpressure >= 1 and meanarterialpressure <= 300 then meanarterialpressure else null end) as meanarterialpressure
, avg(case when arteriallinemap >= 1 and arteriallinemap <= 300 then arteriallinemap else null end) as arteriallinemap
...
OR arteriallinemap IS NOT NULL
